...
#  This can be useful for identifying patterns in the data that are not apparent from the column id alone.
#
# Here, we first create a key for each range by concatenating its lower and upper bounds with a hyphen.
...
#
# With this variation, you can identify which ranges of values are most common in each column
# of your data. For example, you might find that a certain column tends to have a large number
# of low values or a narrow range of values. This can help you to better understand the distribution
# of your data and to identify any potential issues or opportunities for improvement.
...
def RULE(ranges, max_size):
    """
    Groups a list of Range objects by their 'at' attribute, and returns a dictionary of lists of ranges.
    """
    ...
    for range_obj in ranges:
        # If this is the first time we've seen this attribute value, create a new list for it in the dictionary.
        if range_obj.at not in groups:
            groups[range_obj.at] = []
        # Add the current range object to the list for this attribute value.
        groups[range_obj.at].append({"txt": range_obj.txt, "lo": range_obj.lo, "hi": range_obj.hi})
