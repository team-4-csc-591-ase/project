...
from src.contrast_sets import xpln, selects
...
def test_table():
    ...
    for file in files:
        ...
        table1_dict = {'all': [], 'sway1': [], 'sway2': [], 'xpln1': [], 'xpln2': [], 'top': []}

        table2_rows = [[['all', 'all'], None],
                       [['all', 'sway1'], None],
                       [['sway1', 'sway2'], None],
                       [['sway1', 'xpln1'], None],
                       [['sway2', 'xpln2'], None],
                    ...
                    for k in range(len(data.cols.y)):
                        try:
                            if table2_rows[i][1][k] == '=':
                                y0, z0 = table1_dict[base][iteration].cols.y[k], table1_dict[diff][iteration].cols.y[k]
                                is_equal = bootstrap(y0.col.has, z0.col.has) and cliffs_delta(y0.col.has, z0.col.has)
                                if not is_equal:
                                    table2_rows[i][1][k] = '≠'
                        except IndexError:
                            break
